...
DATASET_ROOT = "sprint-final/multileaf_dataset"
...
def safe_int(text):
    try:
        ...
        if text.lower() in ["nava", "napp", "nan", "none", "null", ""]:
            return -1
...
def get_fold(f):
    ...
    try:
        ...
        resultado = df_folds.loc[df_folds['image_name'] + '.jpg' == file_name, 'fold']
...
def parse_xml_leaves(xml_path, img_shape):
    ...
    for obj in objects:
        ...
        width = obj.find('dimensions/width')
        ...
        if obj.tag.lower() == "leaf":
            leaves.append({
                'index': safe_int(index_node.text if index_node is not None else -1),
                ...
                'width': safe_float(width.text if width is not None else -1)
                })
...
def estimate_length_width_px(mask):
    ...
    x, y, w, h = cv2.boundingRect(proj.astype(np.int32))
...
def find_marker_by_geometry(mask):
    ...
    for cnt in contours:
        ...
        # descarta objetos muito alongados
        ...
        # seleciona o maior preenchimento
...
for root, _, files in os.walk(DATASET_ROOT):
    for num, f in enumerate(files):
            ...
            # Detecta componentes previstas que cobrem múltiplas folhas GT
            os.makedirs("merged_debug", exist_ok=True)

            for pred_idx, pred_mask in enumerate(pred_comps):
                folhas_gt = []
                gt_masks_envolvidas = []

                for gt_leaf in gt_leaves:
                    inter = np.logical_and(pred_mask, gt_leaf["mask"]).sum()

                    # limiar para ignorar pequenas interseções
                    if inter > 0:
                        folhas_gt.append(gt_leaf["index"])
                        gt_masks_envolvidas.append(gt_leaf)

                    if len(folhas_gt) > 1:

                        # ---------- registra para o CSV ----------
                        merged_leaf_records.append({
                            "image_name": f,
                            "pred_component": pred_idx,
                            "merged_leaf_indices": ",".join(map(str, folhas_gt)),
                            "num_leaves": len(folhas_gt),
                        })

                        print(
                            f"🚨 {f}: componente prevista {pred_idx} juntou as folhas {folhas_gt}"
                        )
                        ...
                        # componente prevista em vermelho (overlay)
                        overlay = vis.copy()
                        overlay[pred_mask > 0] = (0, 0, 255)
                        vis = cv2.addWeighted(overlay, 0.3, vis, 0.7, 0)

                        # desenha cada folha GT envolvida
                        for gt_leaf in gt_masks_envolvidas:

                            cnts, _ = cv2.findContours(
                                gt_leaf["mask"].astype(np.uint8),
                                cv2.RETR_EXTERNAL,
                                cv2.CHAIN_APPROX_SIMPLE,
                            )
                            ...
                            # escreve o índice da folha
                            ys, xs = np.where(gt_leaf["mask"] > 0)
                            if len(xs) > 0:
                                cx = int(xs.mean())
                                cy = int(ys.mean())
                        ...
                        out_name = (
                            f.replace(".jpg", "") +
                            f"_pred_component_{pred_idx}.png"
                        )

                        cv2.imwrite(
                            os.path.join("merged_debug", out_name),
                            vis,
                        )
            ...
            #pixel_area = w * h
            #marker_perimeter_px = 2 * (w + h)
            marker_perimeter_px = cv2.arcLength(marker_cnt, True)
            marker_perimeter_real = 4 * side


            annot_rect = cv2.minAreaRect(annot_cnt)
            (_, _), (w_annot, h_annot), _ = annot_rect

            #pixel_annot_area = w_annot * h_annot
            annot_marker_perimeter_px = cv2.arcLength(annot_cnt, True)

            marker_side_px = marker_perimeter_px / 4
            annot_marker_side_px = annot_marker_perimeter_px / 4
            ...
            annot_dict = {
                i: (annot_j, annot_score)
                for i, annot_j, annot_score in annot_matches
            }
            ...
            for i, j, score in matches:
                ...
                annot_j, annot_score = annot_dict.get(i, (-1, 0))
                ...
                pred_area_px = mask_area(pred_mask)
                pred_area_cm2 = pred_area_px * scale
                
                gt_area_cm2 = float(gt_leaves[i]['area'])
      
                annot_area_px = mask_area(annot_mask)
                annot_area_cm2 = annot_area_px * annot_scale

                gt_perim_cm = float(gt_leaves[i]['perimeter'])
                pred_perim_cm = (mask_perimeter(pred_mask) / marker_perimeter_px) * marker_perimeter_real
                annot_perim = (mask_perimeter(annot_mask)/ annot_marker_perimeter_px) * marker_perimeter_real

                length_px, width_px = estimate_length_width_px(pred_mask)
                annot_length_px, annot_width_px = estimate_length_width_px(annot_mask) #resolver isso aqui

                pred_length_cm = (length_px / marker_side_px) * side
                pred_width_cm  = (width_px  / marker_side_px) * side

                annot_length = (annot_length_px / annot_marker_side_px) * side
                annot_width  = (annot_width_px  / annot_marker_side_px) * side
          
                gt_length_cm = gt_leaves[i]['length']
                gt_width_cm = gt_leaves[i]['width']
                ...
                if(gt_area_cm2 != -1):
                    ...
                    if area_rer > 0.5:
                        ...
                        print(gt_leaves[i]['index'], area_rer)
                ...
                if gt_width_cm is not None and gt_width_cm > 0:
                    width_rer = abs(pred_width_cm - gt_width_cm) / (gt_width_cm + 1e-8)
                    width_annot_rer = abs(annot_width - gt_width_cm) / (gt_width_cm + 1e-8)
                else:
                    width_rer = np.nan
                    width_annot_rer = np.nan
                ...
                width_method_rer = abs(pred_width_cm - annot_width)/(annot_width + 1e-8)
                ...
                records.append({
                    ...
                    "real_width_cm": gt_width_cm,
                    "pred_width_cm": pred_width_cm,
                    "width_rer": width_rer,
                    "width_annot": annot_width,
                    "width_annot_rer": width_annot_rer,
                    ...
                    'width_method_rer': width_method_rer
...
merged_csv_path = "merged_leaves.csv"
...
with open(merged_csv_path, "w", newline="") as f:
    ...
    writer.writeheader()
    writer.writerows(merged_leaf_records)

print("CSV de folhas mescladas salvo em:", merged_csv_path)
